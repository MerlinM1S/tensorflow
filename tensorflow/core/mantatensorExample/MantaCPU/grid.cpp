...
template<class T>
Grid<T>::Grid(FluidSolver* parent, T* data, bool show) : GridBase(parent), mData(data), extData(true)
{
        mType = typeList<T>();
        mSize = parent->getGridSize();
...
template<class T>
Grid<T>::Grid(const Grid<T>& a) : GridBase(a.getParent()), extData(false) {
	mSize = a.mSize;
	mType = a.mType;
	mStrideZ = a.mStrideZ;
	mDx = a.mDx;
...
template<class T> T tGetValue(Grid<T>& me, int i, int j, int k) {
    return me(i, j, k, 0);
}
template<class T> T Grid<T>::getValue(int i, int j, int k) {
        return tGetValue(*this, i, j, k);
}


template<class T> void tSetValue(Grid<T>& me, int i, int j, int k, T value) {
    me(i, j, k, 0) = value;
}
template<class T> void Grid<T>::setValue(int i, int j, int k, T value) {
    tSetValue(*this, i, j, k, value);
}
...
void convertMacToVec3(MACGrid &source , Grid<Vec3> &target) { debMsg("Deprecated - do not use convertMacToVec3... use copyMacToVec3 instead",1); copyMacToVec3(source,target); }
...
void convertLevelsetToReal(LevelsetGrid &source , Grid<Real> &target) { debMsg("Deprecated - do not use convertLevelsetToReal... use copyLevelsetToReal instead",1); copyLevelsetToReal(source,target); }
...
Real getUvWeight(Grid<Vec3> &uv) { return uv[0][0]; }
...
void getComponent(const Grid<Vec3>& source, Grid<Real>& target, int component) { knGetComponent(source, target, component); }
...
void setComponent(const Grid<Real>& source, Grid<Vec3>& target, int component) { knSetComponent(source, target, component); }
