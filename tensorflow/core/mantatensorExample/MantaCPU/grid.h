...
template<class T> class Grid : public GridBase {public:
	...
	//! access data
	inline void set(int i,int j, int k, T v)       { mData[index(i,j,k)] = v; }
	...
	Grid<T>& copyFrom(const Grid<T>& a, bool copyType=true ); // old: { *this = a; }
...
//	FlagGrid(FluidSolver* parent, int* data) : Grid<int>(parent, data) { }
