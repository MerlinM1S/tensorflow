...
#line 1 "/home/ansorge/workspace_master/manta/source/particle.h"
/******************************************************************************
 *
...
template<class S> class ParticleSystem : public ParticleBase {public:    
	...
	int pySize() const { return (int)mData.size(); }
	...
	void setPos(const IndexInt idx, const Vec3& pos) { DEBUG_ONLY(checkPartIndex(idx)); mData[idx].pos = pos; }
	Vec3 getPos(IndexInt idx) const { DEBUG_ONLY(checkPartIndex(idx)); return mData[idx].pos; }
...
void advectInGrid( const FlagGrid& flags, const MACGrid& vel, const int integrationMode, const bool deleteInObstacle=true, const bool stopInObstacle=true, const ParticleDataImpl<int> *ptype=NULL, const int exclude=0);
	
	//! Project particles outside obstacles
	void projectOutside(Grid<Vec3>& gradient);
	void projectOutOfBnd(const FlagGrid &flags, const Real bnd, const std::string& plane="xXyYzZ", const ParticleDataImpl<int> *ptype=NULL, const int exclude=0);
...
class BasicParticleSystem : public ParticleSystem<BasicParticleData> {public:
	...
	void addParticle(Vec3 pos) { add(BasicParticleData(pos)); }
...
protected:
	std::vector<CON> mSegments; 	virtual void compress();     }
;
...
protected: 	ParticleBase* mpParticleSys; }
;
...
template<class T> class ParticleDataImpl : public ParticleDataBase {public:
	...
	void setConstIntFlag(T s, const ParticleDataImpl<int>& t, const int flag);
...
#pragma omp parallel 
 {  
#pragma omp for  
  for (IndexInt i = 0; i < _sz; i++) op(i,p,vel,flags,dt,deleteInObstacle,stopInObstacle,ptype,exclude,u);  }   } std::vector<S>& p; const MACGrid& vel; const FlagGrid& flags; Real dt; bool deleteInObstacle; bool stopInObstacle; const ParticleDataImpl<int> * ptype; const int exclude;  std::vector<Vec3>  u;  };
#line 428 "particle.h"
